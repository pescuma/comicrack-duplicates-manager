    ...
    if with_covers != []:
        ...
        for (comic,covers) in with_covers:
            if covers < max:
               ...
               logfile.write('removed... '+ comic[3]+'\n')
            else:
               logfile.write('keeping... '+ comic[3]+'\n')
...
def keep_filesize_largest(item, dgroup, logfile):
    ...
    for comic in by_size[1:]:
        ...
        logfile.write('removed... '+ comic[3]+'(size '+comic[item]+')\n')
...
def keep_pagecount_largest(item, dgroup, logfile):
    ...
    for comic in by_size:
        logfile.write(comic[3]+' (pages '+comic[item]+')\n')
    for comic in by_size[1:]:
        logfile.write(comic[3]+' (pages '+comic[item]+')\n')
    ...
    for comic in by_size[1:]:
        dgroup.remove(comic)
        logfile.write('removed... '+ comic[3]+' (pages '+comic[item]+')\n')

    logfile.write('keeping... '+ by_size[0][3]+' (pages '+by_size[0][item]+')\n') 
